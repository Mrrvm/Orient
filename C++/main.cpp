...
int main() {
    ...
    while(1) {
        ...
        cout << BLUE << "IMU (quat2)" << RESET << quat2 << endl;
        cout << BLUE << "IMU (quat2)" << RESET << quat1 << endl;
        cout << BLUE << "IMU (rotation eul)" << RESET << Quat2Eul(QuatMultiply(quat2, QuatConjugate(quat1)))*180/M_PI << endl;
        quat2.copyTo(quat1);
        getchar();
