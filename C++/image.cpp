...
bool Image::FindMatches(Image img1, Image img2, Mat& m1, Mat& m2, vector<DMatch>& matches) {
    ...
    for(int i = 0; i < int(matches.size()); i++) {
        if(matches[i].distance <= max(2*minDist, 0.02)) {
            m1.at<double>(i, 1) = img1.keypoints[matches[i].queryIdx].pt.x;
            m1.at<double>(i, 2) = img1.keypoints[matches[i].queryIdx].pt.y;
            m2.at<double>(i, 1) = img2.keypoints[matches[i].queryIdx].pt.x;
            m2.at<double>(i, 2) = img2.keypoints[matches[i].queryIdx].pt.y;
        }
    }
    return m1.rows >= 3;
}
